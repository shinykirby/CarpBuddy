...
class FishingBot:
    ...
    def detect_minigame(self, haystack_img):
        ...
        if max_val > 0.9:
            print("Fishing minigame detected!")
            return True
    ...
    def runHack(self):
        ...
        # Save a screenshot at the start of each state
        cv.imwrite(f"state_{self.state}_screenshot.jpg", screenshot)

        # crop and apply hsv filter
        ...
        if self.state == 1:
            if time() - self.timer_action > self.throw_time:
                print("throwing fishing line out to the sea!")
        ...
        if detected_end == True:
            print("DETECTING FISHING MINIGAME")
        ...
        if (time() - self.timer_mouse) > 0.3 and self.state == 3 and detected_end:
            ...
            print("trying to detect the fish")
            # Detect the fish            
